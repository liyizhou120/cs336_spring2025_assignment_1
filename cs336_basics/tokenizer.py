...
class BPETokenizer:
    ...
    def encode_to_npfile(self, input_path: os.PathLike, output_path: os.PathLike) -> None:
        ...
        # 保存并重置缓存
        original_cache = self.word_to_ids
        self.word_to_ids = {}
        
        # 创建临时文件（不使用 with 语句，手动管理）
        ...
        try:
            ...
            with open(input_path, 'r', encoding='utf-8') as f:
                pbar = tqdm(f, desc="编码进度", unit="行")
                
                for line in pbar:
                    # 编码这一行
                    words = self.pre_tokenizer.pretokenize(line)
                    
                    for word in words:
                        # 查找或计算token IDs
                        if word in self.token_to_id:
                            token_ids = [self.token_to_id[word]]
                        elif word in self.word_to_ids:
                            token_ids = self.word_to_ids[word]
                        else:
                            token_ids = self.calculate_token_ids(word)
                            # 限制缓存大小
                            if len(self.word_to_ids) < 10000:
                                self.word_to_ids[word] = token_ids
            ...
            # 恢复原始缓存
            self.word_to_ids = original_cache
...
if __name__ == "__main__":
    ...
    #input_path = "/media/yizhouli/1TB 970 Evo Plus/code/cs336/data/TinyStoriesV2-GPT4-valid.txt"
    ...
    output_path = "/media/yizhouli/1TB 970 Evo Plus/code/cs336/data/TinyStories_train_10000_token_ids.npy"
